...
grid = BeckeMolGrid(mol.coordinates, mol.numbers,
                    mol.pseudo_numbers, agspec, mode='only')
dm_full = mol.get_dm_full()
moldens = mol.obasis.compute_grid_density_dm(
    dm_full, grid.points, epsilon=1e-8)

# do partitioning depending on method
if method == "hi":
    try:
        with open('atoms.h5') as f:
            pass
    except IOError as e:
...
if method == "mbis":
    ...
    wpart = WPartClass(mol.coordinates, mol.numbers,
